...
class Bamboo private (val ssBlocks: SSBlocks) {
  ...
  import Bamboo.{sample,oneDiff,twoDiff,toSequence,minLengths,possibilities}
  ...
  if ( nBlocks > 0 ) {
    if ( ssBlocks.head._1 != "C" ) throw new IllegalArgumentException("Sequence must begin in coil.")
    if ( ssBlocks.last._1 != "C" ) throw new IllegalArgumentException("Sequence must end in coil.")
    if ( ssBlocks.forall(_._1 == "C") ) throw new IllegalArgumentException("Sequence cannot be all coil.")
    ssBlocks.foreach(x => {
      if ( ! minLengths.contains(x._1) ) throw new IllegalArgumentException("Block types must be H, T, E, or C.")
      val min = minLengths(x._1)
      if ( x._2 < min ) throw new IllegalArgumentException("Blocks must have length 5, 3, 3, or 1, respectively.")
    })
...
object Bamboo {
  ...
  private val BlockReg = """^(H+|E+|T+|C+)(.*)$""".r
    ...
    while ( xx.length > 0 ) {
      xx match {
        case BlockReg(block,residue) =>
          ...
          yy = (tt,len) :: yy
          xx = residue
        case _ => throw new IllegalArgumentException("Violation of secondary structure syntax.")
      }
    }
    yy
  }
  ...
  def enumerate(length : Int) = {
    ...
    def expand(fixedSSSeq: SSBlocks, fixedLength: Int, block: Block): Unit = {
      if ( fixedLength + block._2 == length ) {
        ...
        types.filter(_ != block._1).foreach( t => expand(fixedSSSeq :+ block, fixedLength + block._2, (t,1)) )
      }
    }
  ...
  private val possibilities = Map("H"->1.0,"E"->1.0,"T"->1.0,"C"->1.0)
  private val minLengths = Map("H"->5,"E"->3,"T"->3,"C"->1)
  private val types = List("H","E","T","C")
